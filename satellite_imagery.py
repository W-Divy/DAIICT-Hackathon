...
import requests
import numpy as np
import matplotlib.pyplot as plt# INSTANT SATELLITE DATA SOLUTION FOR VEGETATION HEALTH MONITORING
import requests
import numpy as np
...
def send_whatsapp_message(phone_no, message):
    """
    Send WhatsApp message with session preservation
    """
    # First, check if we need to login
    if not os.path.exists("./.whatsapp_session"):
        print("Please scan the QR code when prompted...")
        # This will open WhatsApp Web and you'll need to scan QR code
        pwk.sendwhatmsg_instantly(phone_no, "Initializing session...", 15, True, 3)
        
        # Create session file to indicate we've logged in
        with open("./.whatsapp_session", "w") as f:
            f.write("session_active")
        
        print("Session established. Future messages won't require login.")
    else:
        # Already logged in, send message directly
        try:
            now = time.localtime()
            hour = now.tm_hour
            minute = now.tm_min + 1
            
            pwk.sendwhatmsg(phone_no, message, hour, minute, 10, True, 2)
            print("Message sent successfully!")
        except Exception as e:
            print(f"Error sending message: {e}")

# Example usage
send_whatsapp_message("+916351137369", "🚨 AI Alert: System issue detected!")
...
if health_stats['health_status'] == 'Fair':
    send_whatsapp_message("+916351137369", "🚨 AI Alert: Anomaly detected!")
